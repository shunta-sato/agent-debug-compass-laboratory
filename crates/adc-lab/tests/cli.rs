...
fn collect_plan_writes_v2_argv_steps_and_markdown() {
    ...
    let prepare_governor_retrieval_parent_step =
        step_by_id(steps, "prepare_target_local_governor_retrieval_parent");
    let reset_governor_retrieval_destination_step =
        step_by_id(steps, "reset_target_local_governor_retrieval_destination");
    let retrieve_governor_step = step_by_id(steps, "retrieve_target_local_governor_sweep");
    let retrieved_governor_dir =
        format!("{}/included/target-local-governor-sweep", run_dir.display());
    assert!(
        step_index(steps, "governor_sweep_run")
            < step_index(steps, "prepare_target_local_governor_retrieval_parent")
    );
    assert!(
        step_index(steps, "prepare_target_local_governor_retrieval_parent")
            < step_index(steps, "reset_target_local_governor_retrieval_destination")
    );
    assert!(
        step_index(steps, "reset_target_local_governor_retrieval_destination")
            < step_index(steps, "retrieve_target_local_governor_sweep")
    );
    assert!(
        step_index(steps, "retrieve_target_local_governor_sweep")
            < step_index(steps, "run_validation")
    );
    assert_eq!(
        prepare_governor_retrieval_parent_step["execution_location"],
        "operator_handoff"
    );
    assert_eq!(
        prepare_governor_retrieval_parent_step["command_argv"]
            .as_array()
            .unwrap(),
        &[
    ...
    assert_eq!(
        reset_governor_retrieval_destination_step["execution_location"],
        "operator_handoff"
    );
    assert_eq!(
        reset_governor_retrieval_destination_step["command_argv"]
            .as_array()
            .unwrap(),
        &[
            serde_json::Value::String("rm".to_string()),
            serde_json::Value::String("-rf".to_string()),
            serde_json::Value::String(retrieved_governor_dir.clone())
        ]
    );
    assert_eq!(
        retrieve_governor_step["execution_location"],
        "operator_handoff"
    );
    let retrieve_governor_argv = retrieve_governor_step["command_argv"].as_array().unwrap();
    assert_eq!(retrieve_governor_argv[0], "scp");
    assert_eq!(retrieve_governor_argv[1], "-r");
    assert_eq!(
        retrieve_governor_argv[2],
        "target55:adc-lab-target-local-run"
    );
    assert_eq!(
        retrieve_governor_argv[3],
        serde_json::Value::String(retrieved_governor_dir.clone())
    );
    assert_eq!(
        retrieve_governor_step["expected_artifact_paths_or_globs"]
            .as_array()
            .unwrap(),
        &[serde_json::Value::String(retrieved_governor_dir.clone())]
    );
    ...
    let workload_target_index = workload_argv
        ...
        .unwrap()
        + 1;
    assert_eq!(workload_argv[workload_target_index], "local");
    assert!(!workload_argv.iter().any(|arg| arg == "ssh://target55"));
    let workload_run_dir_index = workload_argv
        .iter()
        .position(|arg| arg == "--run-dir")
        .unwrap()
        + 1;
    assert_eq!(
        workload_argv[workload_run_dir_index],
        "adc-lab-target-local-workload-run"
    );
    let workload_plan_index = workload_argv
        .iter()
        .position(|arg| arg == "--plan")
        .unwrap()
        + 1;
    assert_eq!(
        workload_argv[workload_plan_index],
        "adc-lab-target-local-workload-run/inputs/workload_run_plan.yaml"
    );
    assert_eq!(
        argv_values(workload_argv, "--execution-mode"),
        vec!["target-local".to_string()]
    );
    ...
    let retrieved_workload_dir = format!(
    ...
    assert_eq!(
        retrieve_workload_argv[2],
        "target55:adc-lab-target-local-workload-run"
    );
    assert_eq!(
        retrieve_workload_argv[3],
        serde_json::Value::String(retrieved_workload_dir.clone())
    );
    assert_eq!(
        retrieve_workload_step["expected_artifact_paths_or_globs"]
            .as_array()
            .unwrap(),
        &[serde_json::Value::String(format!(
            "{retrieved_workload_dir}/reports/workload_demand_profile.json"
        ))]
    );
    ...
    assert_eq!(
        ...
        vec![retrieved_governor_dir.clone()]
    );
    ...
    let suitability_step = step_by_id(steps, "suitability");
    assert_eq!(
        argv_values(
            suitability_step["command_argv"].as_array().unwrap(),
            "--workload-demand"
        ),
        vec![format!(
            "{retrieved_workload_dir}/reports/workload_demand_profile.json"
    ...
    assert!(instructions.contains("prepare_target_local_governor_retrieval_parent"));
    assert!(instructions.contains("reset_target_local_governor_retrieval_destination"));
...
#[test]
fn decide_suitability_refused_workload_demand_keeps_selection_not_ready() {
    let temp = tempfile::tempdir().unwrap();
    ...
    Command::cargo_bin("adc-lab")
        ...
        .success()
        .stdout(contains("remote_workload_execution_not_supported_in_v1"));
    let demand_path = temp.path().join("reports/workload_demand_profile.json");
    let demand: serde_json::Value =
        serde_json::from_slice(&fs::read(&demand_path).unwrap()).unwrap();
    assert_eq!(demand["data_quality"]["degraded"], true);
    ...
    let decision_path = temp
        .path()
        .join("reports/suitability_refused_workload.json");
    fs::copy(
        workspace_root().join("tests/golden/lab.suitability_policy.v1.valid.json"),
        &policy_path,
    )
    ...
    assert_eq!(decision["payload"]["selection_ready"], false);
    assert_eq!(decision["data_quality"]["level"], "degraded");
    let dimensions = decision["payload"]["dimensions"].as_array().unwrap();
    for dimension in ["cpu", "memory"] {
        let entry = dimensions
            .iter()
            .find(|entry| entry["dimension"] == dimension)
            .unwrap();
        assert_eq!(entry["decision"], "unknown");
        assert!(entry["unknown_reason"]
            .as_str()
            .is_some_and(|reason| reason.contains("workload run did not complete cleanly")));
    }
}

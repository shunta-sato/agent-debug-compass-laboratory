...
| CPU | no always-on default | bounded by worker count and duration | demo target55 idle/load smoke | default over budget blocks submit | demo-partial |
| Wakeups | no always-on default | bounded by sample interval | not measured | unbounded wakeups block submit | unknown |
...
| Thermal | no default heat claim | thermal abort supported | demo target55 short load smoke | feature-caused thermal rise needs degraded mode | demo-partial |
...
| `adc-lab load cpu` | experimental-only burst | worker loop | command duration | no | demo target55 thermal/load smoke captured |
...
| thermal_pressure | CPU load supports abort temperature | demo target55 short smoke only |

...
- Platform Operating Contract discovery: command-triggered pressure probes for
  ...
  `lab.target_operating_contract.v1`. They are short-smoke/experimental until
  repeated under controlled operating points and approved longer soak windows.
...
| RSS / heap | unknown | bounded anonymous memory pressure command | memory pressure probe touches bounded memory | unbounded growth blocks submit | measured-partial |
...
| Storage writes | no continuous default writes | bounded tempfile storage probe with cleanup verification | code review plus pressure artifact | continuous target writes need budget | measured-partial |
...
| Network/radio | no hidden background use | SSH fixed commands plus network pressure counter probe | demo target55 smoke over SSH and pressure artifact | hidden background use blocks battery-safe claim | measured-partial |
...
| Latency / jitter | no production latency claim | monotonic jitter loop under current condition | pressure artifact | missed timing budget blocks submit | measured-partial |
| Observer overhead | no production overhead claim | short observer-on/off comparison plus observer pressure probe | demo target55 short smoke and pressure artifact | uncalibrated overhead limits claims | measured-partial |
...
| `adc-lab pressure run` | experimental-only burst | probe-specific bounded loop or I/O window | duration <=30s; memory <=128MiB; storage <=64MiB; network <=1MiB | no | hardware-free CLI/schema tests; target55 run required for target claims |
...
| network_unavailable | record interface counters if visible; classify endpoint-specific traffic as not_applicable/not measured rather than unsupported | pressure probe artifact |
| latency_pressure | degrade real-time-ish claims unless p95/p99/max jitter evidence exists for the pressure condition | pressure probe artifact |
...
- Remaining evidence gaps after pressure smoke: wakeups, battery/power, flash
  wear, sustained thermal/recovery/degraded envelope, controlled governor and

...
- [x] PR 4: Deep CPU / thermal characterization profile.
  ...
  - [x] Run review-fix verification.
  - [x] Merge PR #67.
- [x] PR 5: Pressure / composite / endpoint-backed network coverage.
...
- [ ] PR 6: Suitability dimension linkage.
  ...
  - [x] Add target-run pressure evidence summary extraction for v2 pressure
        artifacts.
  - [x] Add storage/network/latency suitability decisions for matching evidence.
  - [x] Keep counter-only network unknown for required network suitability.
  - [x] Keep tempfile/page-cache storage smoke marginal, not storage-device meet.
  - [x] Keep missing required latency unknown and blocking `selection_ready`.
...
PR 6 dev-workflow route:

- Risk: normal Agent-facing claim-boundary change. It changes downstream
  suitability interpretation of existing typed pressure artifacts, but does not
  add new pressure primitives, generated argv, privileged behavior, or claim
  relaxation to `meet`.
- Triggered branches: execution-plan update, implementation-economy,
  agent-workflow-contract-review, focused suitability tests, CLI suitability loop
  regression tests, file-budget enforcement, full `make verify`.
- Non-triggered branches: no concurrency, no UI, no arbitrary remote command, no
  target-local daemon/hot path.

PR 6 implementation-economy budget:

- Changed production files target: 1 core file, plus integration tests, plan, and
  workflow-contract review report.
- New modules target: 0.
- New helper target: one small target-run pressure summary type plus local
  decision helpers.
- Public schema/API target: no artifact schema changes.
- File-budget target: keep `suitability.rs` under 1500 lines by moving
  module-local tests to `crates/adc-lab-core/tests/suitability.rs`.

PR 6 implementation-economy audit:

| New abstraction | Justification | Decision | Evidence |
|---|---|---|---|
| `TargetRunPressureEvidence` | Captures the subset of v2 pressure payload semantics needed by suitability without teaching suitability to infer from filenames or raw directory co-presence. | keep | Integration tests cover storage, network, latency, counter-only network, and missing required latency. |
| Storage/network/latency decision helpers | Keeps each resource dimension's sufficiency boundary explicit and reviewable. | keep | Storage smoke stays marginal, counter-only network stays unknown, endpoint-backed bounded transfer becomes non-unknown only with generated bytes. |
| Integration test file `tests/suitability.rs` | Preserves the enforced production file budget after adding the resource evidence sufficiency matrix. | keep | `check-file-budgets.py --enforce` passes with `suitability.rs` at 1359/1500 lines. |

Budget note:

PR6 intentionally does not add `report.resource_evidence_summary.v2`; the
decision logic stayed narrow enough to summarize pressure artifacts privately
inside suitability. If future resource dimensions require richer aggregation or
cross-run weighting, that report should be introduced before expanding raw scan
logic further.
...
- 2026-06-16: Implement PR6 suitability linkage by privately summarizing v2
  pressure artifacts instead of adding `report.resource_evidence_summary.v2`.
  Rationale: storage/network/latency linkage can be satisfied from the existing
  pressure payload contract without another public artifact surface.
- 2026-06-16: Keep storage/network/latency linkage conservative: endpoint-backed
  bounded transfer and measured latency/storage evidence can avoid `unknown`,
  but storage smoke, current-condition latency, and endpoint network without
  policy thresholds remain `marginal`, never `meet`. Rationale: PR6 links
  evidence to dimensions; it does not create device/SLO/production claims.
- 2026-06-16: Move suitability unit coverage into
  `crates/adc-lab-core/tests/suitability.rs` rather than adding a file-budget
...
PR 6 focused verification:

- `cargo test -p adc-lab-core --test suitability -- --nocapture`: pass (7 tests).
- `cargo test -p adc-lab-core suitability_ -- --nocapture`: pass (moved
  suitability integration coverage plus existing conservative rules regression).
...
- `cargo test -p adc-lab --test cli suitability_and_constraints_refs_resolve_across_included_run_set -- --nocapture`:
  pass.
- `python3 scripts/ci/check-file-budgets.py --enforce`: pass (`file budgets:
  enforced checked=59 violations=0`; `suitability.rs` 1359/1500).
- `git diff --check`: pass.
- `make verify`: pass (`file budgets: enforced checked=59 violations=0`; CLI
  54 tests; safety invariant tests 9 + 19; suitability integration 7; schema
...
- Decision: submit.
- Findings: 0 from workflow-contract review report
  `reports/workflow-contract-review/v024-pr6-suitability-linkage.md`.
